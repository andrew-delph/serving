...
func computeActiveCondition(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler, pc podCounts) {
	...
	// Early check for cases when the Service is not reconciled yet.
	if isServiceCreated == false {
		if pa.Status.IsScaleTargetInitialized() == false && minReady == 0 {
	...
	case pc.want == 0:
		...
		if pa.Status.IsActivating() {
			...
			// The pa is no longer recieving traffic.
			pa.Status.MarkInactive(noTrafficReason, "The target is not receiving traffic.")
		}
	...
	case pc.want < 0:
		// The decider is not recieving metrics because the pa scaled to zero.
		pa.Status.MarkInactive(noTrafficReason, "The target is not receiving traffic.")
