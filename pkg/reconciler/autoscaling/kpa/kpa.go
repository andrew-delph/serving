...
func computeActiveCondition(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler, pc podCounts) {
	switch {
	case pa.Status.IsActive():
		computeActiveConditionFromActive(ctx, pa, pc)
	case pa.Status.IsInactive():
		computeActiveConditionFromInactive(ctx, pa, pc)
	case pa.Status.IsInactive():
...
func computeActiveConditionFromActive(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler, pc podCounts) {
	minReady := activeThreshold(ctx, pa)
	if pc.ready >= minReady && pa.Status.ServiceName != "" {
		pa.Status.MarkScaleTargetInitialized()
	}
...
func computeActiveConditionFromInactive(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler, pc podCounts) {
	minReady := activeThreshold(ctx, pa)
...
func computeActiveConditionFromActivating(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler, pc podCounts) {
	minReady := activeThreshold(ctx, pa)
	if pc.ready >= minReady && pa.Status.ServiceName != "" {
		pa.Status.MarkScaleTargetInitialized()
	}
